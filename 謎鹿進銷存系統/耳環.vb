...
Public Class 耳環
    ...
    Private Sub Button6_Click(sender As Object, e As EventArgs) Handles Button6.Click
        ...
        購物車.Label48.Text = Val(購物車.Label14.Text) + Val(購物車.Label15.Text) + Val(購物車.Label16.Text) + Val(購物車.Label17.Text) + Val(購物車.Label51.Text)
    End Sub
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click '新增購物車紀錄
        conn.Open()

        購物車.Label10.Text = Val(購物車.Label10.Text) + Val(TextBox2.Text)
        購物車.Label11.Text = Val(購物車.Label11.Text) + Val(TextBox3.Text)
        購物車.Label12.Text = Val(購物車.Label12.Text) + Val(TextBox4.Text)
        購物車.Label13.Text = Val(購物車.Label13.Text) + Val(TextBox5.Text)
        ...
        購物車.Label46.Text = Val(購物車.Label46.Text) + Val(手工餅乾.TextBox3.Text)

        購物車.Label14.Text = 290 * TextBox2.Text
        購物車.Label15.Text = 190 * TextBox3.Text
        購物車.Label16.Text = 190 * TextBox4.Text
        購物車.Label17.Text = 290 * TextBox5.Text
        ...
        購物車.Label45.Text = 30 * 手工餅乾.TextBox3.Text
        ...
        Dim m_會員編號 = Label6.Text
        Dim m_運費 = 購物車.Label51.Text
        Dim m_總金額 = 購物車.Label48.Text
        Dim m_購物車編號 = Label6.Text + "cart"

        Dim sqladd As String = "INSERT INTO 購物車(會員編號,運費,總金額,購物車編號) VALUES('" & Label6.Text & "','" & 購物車.Label51.Text & "','" & 購物車.Label48.Text & "',,'" & Label6.Text + "cart" & "')"
    ...
    Private Sub 耳環_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'TODO: 這行程式碼會將資料載入 '謎鹿DataSet.購物車' 資料表。您可以視需要進行移動或移除。
        Me.購物車TableAdapter.Fill(Me.謎鹿DataSet.購物車)
        Label6.Text = 登入.TextBox1.Text
        Label20.Text = Format(Now, "yyMMdHHmmss")

        Dim myReader1 As SqlDataReader
        Dim mySqlCommand1 As SqlCommand

        mySqlCommand1 = New SqlCommand("SELECT 庫存數量 from 商品 WHERE 商品編號= '" & Label25.Text & "'", conn)
        Try
            conn.Open()
            myReader1 = mySqlCommand1.ExecuteReader()
            Do While (myReader1.Read())
                Label21.Text = myReader1("庫存數量").ToString() '→給Label值
            Loop
        Catch ex As Exception
        ...
        mySqlCommand1 = New SqlCommand("SELECT 庫存數量 from 商品 WHERE 商品編號= '" & Label26.Text & "'", conn)
        Try
            conn.Open()
            myReader1 = mySqlCommand1.ExecuteReader()
            Do While (myReader1.Read())
                Label22.Text = myReader1("庫存數量").ToString() '→給Label值
            Loop
        Catch ex As Exception
            MessageBox.Show(ex.ToString())
        Finally
        ...
        mySqlCommand1 = New SqlCommand("SELECT 庫存數量 from 商品 WHERE 商品編號= '" & Label27.Text & "'", conn)
        Try
            conn.Open()
            myReader1 = mySqlCommand1.ExecuteReader()
            Do While (myReader1.Read())
                Label23.Text = myReader1("庫存數量").ToString() '→給Label值
            Loop
        Catch ex As Exception
            MessageBox.Show(ex.ToString())
        Finally
        ...
        mySqlCommand1 = New SqlCommand("SELECT 庫存數量 from 商品 WHERE 商品編號= '" & Label28.Text & "'", conn)
        Try
            conn.Open()
            myReader1 = mySqlCommand1.ExecuteReader()
            Do While (myReader1.Read())
                Label24.Text = myReader1("庫存數量").ToString() '→給Label值
            Loop
        Catch ex As Exception
            MessageBox.Show(ex.ToString())
        Finally
    ...
    Private Sub Button9_Click(sender As Object, e As EventArgs) Handles Button9.Click '刪除購物車紀錄
        Dim m_會員編號 = Label6.Text
        ...
        conn.Open()
        Dim sqldel As String = "DELETE FROM 購物車 WHERE 會員編號 IN ('" & Label6.Text & "')"
